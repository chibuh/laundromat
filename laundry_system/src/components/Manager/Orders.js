...
export default function Orders() {
  ...
  React.useEffect(() => {
    ...
    function handleDone(index) {
      ...
      if(window.confirm("Confirm wash status?")===true){

        axios.post(`/api/manager/updatewash`,{
        ...
        <TableBody>
          {rows.map((row,index) => (
            <TableRow key={index}>
              ...
              <TableCell><Button variant="outlined" size={'small'} onClick={() => handleDone(index)}>Done</Button></TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
