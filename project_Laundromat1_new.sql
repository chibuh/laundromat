...
('E001','L001','Kshitij','Garg','9696969696','hellokg@gmail.com',200000,'M','2002-04-15','2020-04-15'),
('E002','L001','Urvashi','Darolia','9696969697','helloud0@gmail.com',15000,'F','2001-01-02','2020-04-15'),
('E003','L002','Chirag','Maheshwari','9696969698','hellocm@gmail.com',200000,'M','2005-10-15','2018-01-15'),
('E004','L001','Utkarsh','Darolia','9696969699','helloud@gmail.com',15000,'M','1998-02-10','2021-03-15'),
('E005','L001','Tanveer','Singh','9696969688','hellots@gmail.com',15000,'M','2002-11-06','2020-04-15'),
('E006','L002','Shreyas','Ketkar','9696969689','hellosk@gmail.com',15000,'M','2000-01-23','2020-04-15');
...
INSERT INTO packages values('P005',6,6,2500,'2022-06-06','2022-05-05',False);			-- End date is set befire the start date
...
UPDATE Order_Details SET Actual_Delivery_Date = '2022-04-14', OStatus = true WHERE orderID = 'ORD007';

SELECT orderID, is_overdue(order_details.orderID) as 'IS_OVERDUE?', count_overdue_days(order_details.orderID) as 'NO_OF_DAYS' from order_details;

CREATE VIEW managerViewWorker as
	SELECT * FROM employee where eid in(SELECT eid from worker);	-- manager can view every detail about workers but not other managers

CREATE VIEW managerViewManager as
	SELECT EID,EmpLID,EMPFirstName,EmpLastNAme,EmpPhone,EmpEmail,Sex,EmpDOB from employee where eid in(SELECT managerID from manager);	-- managers cannot view other manager's salary and date of joining

SELECT * FROM managerViewWorker	;	-- list all workers
SELECT * FROM managerViewWorker WHERE empLID = 'L001';		-- list all workers working in Laindromat L001
SELECT * from managerViewManager;	-- list all available details about managers

UPDATE Users SET lastName = 'Gupta' WHERE UID = 'U001';

DELETE FROM packages where PID = 'P001';		-- since this package is corrently active i.e. it is being used by some customer, it cannot be deleted (acheived using ON DELETE RESTRICT)
DELETE FROM employee where EID = 'E006';		-- deleting from the employee table also deletes the records from the corresponding manager or worker table (achieved using ON DELETE CASCADE)
